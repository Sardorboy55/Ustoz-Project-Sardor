...
Future<void> showPackagePickerSheet(
  ...
  required Map<int, int?> prices, // duration(30/60/90) → price in tiyin (null = not offered)
...
class _PackagePickerSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_PackagePickerSheet> createState() => _PackagePickerSheetState();
}

class _PackagePickerSheetState extends ConsumerState<_PackagePickerSheet> {
  late final List<int> _durations = [30, 60, 90]
      .where((d) => (widget.prices[d] ?? 0) > 0)
      .toList();
  ...
  Future<void> _proceed() async {
    ...
    try {
      final row = await ref.read(paymentRepositoryProvider).ensurePackagePayment(
      ...
      if (!mounted) return;
      setState(() {
        _payAmount = (row?['pay_amount'] as num?)?.toInt() ??
  ...
  Future<void> _upload() async {
    ...
    if (await file.length() > 10 * 1024 * 1024) {
      setState(() =>
          _error = _ru ? 'Файл больше 10 МБ' : 'Fayl 10 MB dan katta');
      return;
    }
    setState(() {
      _uploading = true;
    ...
    try {
      ...
      await repo.submitPackageProof(path);
      if (mounted) {
        setState(() {
          _uploading = false;
          _sent = true;
      ...
      if (mounted) {
        setState(() {
          _uploading = false;
          _error = _ru ? 'Не удалось отправить чек' : 'Chek yuborilmadi';
        });
      }
    }
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                if (_step == 0) ..._selectStep(ru, locale) else _payStep(ru),
              ],
            ),
          ),
        ),
  ...
  List<Widget> _selectStep(bool ru, Locale locale) {
    if (_durations.isEmpty) {
      return [
        Text(ru ? 'Пакеты недоступны для этого предмета' : 'Bu fan uchun paket yo\'q'),
      ];
    }
    final total = _totalTiyin(_duration!, _size);
    final perLesson = (total / _size).round();
    return [
      Text(ru ? 'Длительность урока' : 'Dars davomiyligi',
      ...
      const SizedBox(height: AppTokens.s16),
      Text(ru ? 'Сколько уроков' : 'Nechta dars',
          style: const TextStyle(fontWeight: FontWeight.w600)),
      const SizedBox(height: AppTokens.s8),
  ...
  Widget _payStep(bool ru) {
    return QrPaymentView(
      ...
      errorText: _error,
      sentNote: ru
          ? 'После подтверждения уроки зачислятся в «Мои пакеты».'
          : 'Tasdiqdan so\'ng darslar «Mening paketlarim»ga qo\'shiladi.',
      sentAction: FilledButton(
        onPressed: () {
          Navigator.of(context).pop();
          context.push('/packages');
        },
        child: Text(ru ? 'Мои пакеты' : 'Mening paketlarim'),
      ),
    );
  }
}
