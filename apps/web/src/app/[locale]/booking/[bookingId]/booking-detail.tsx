...
export function BookingDetail({ bookingId }: { bookingId: string }) {
  ...
  // Заявка: pending → «на проверке», rejected → причина + перезалив чека.
  ...
  const load = useCallback(async () => {
    ...
    if (bk.status === "pending_payment" && bk.price > 0) {
      // Создаём/получаем заявку с уникальной суммой-кодом + узнаём статус:
      // отправлен ли чек (pending) или отклонён (rejected → причина + перезалив).
      ...
      if (row) {
        ...
        setPayRejected(rejected);
        setReviewNote(rejected ? row.review_note : null);
        // Чек считается отправленным, только если он загружен и не отклонён.
        setProofSent(!rejected && !!row.receipt_path);
      }
    }
    setPhase("ready");
  }, [bookingId, router]);

  // Загрузка чека → submit_payment_proof → «на проверке» (админ подтвердит).
  const onReceipt = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Новый чек после отклонения снова уходит на проверку.
    setPayRejected(false);
    setReviewNote(null);
    setProofErr(null);
    setProofSent(true);
  };
      ...
      <main className="mx-auto w-full max-w-md flex-1 px-4 py-8 sm:px-6 sm:py-10">
        ...
        <Card className="mt-5 p-5 sm:p-6">
          ...
          <div className="mt-6 border-t border-zinc-100 pt-6">
            {proofSent ? (
              <div className="flex flex-col items-center rounded-2xl bg-amber-50 px-5 py-7 text-center">
              ...
              <>
                ...
                {payRejected && (
                  <div className="mt-4 w-full rounded-xl bg-red-50 px-4 py-3.5 text-left">
