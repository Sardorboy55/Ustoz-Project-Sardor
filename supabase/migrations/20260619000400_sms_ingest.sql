...
begin
  ...
  -- ПЛЮС целая часть без копеек/десятых (241.0 → 241; 50 037,00 → 50037) —
  -- иначе «241.0 so'm» давало «2410» и не совпадало ни с суммой, ни с тийинами.
  v_nums := array(
    select regexp_replace(m[1], '\D', '', 'g')
    from regexp_matches(v_sms, '[0-9][0-9 .,]*[0-9]|[0-9]', 'g') as m
  ) || array(
    select regexp_replace(regexp_replace(m[1], '[.,][0-9]{1,2}[[:space:]]*$', ''), '\D', '', 'g')
    from regexp_matches(v_sms, '[0-9][0-9 .,]*[0-9]|[0-9]', 'g') as m
  );
